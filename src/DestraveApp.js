...
function Sidebar({page,setPage}){return <aside><div className="logo">D<sup>✦</sup></div><nav>{nav.map(([id,icon,label])=><button key={id} className={page===id?'on':''} onClick={()=>setPage(id)}><b>{icon}</b><span>{label}</span></button>)}</nav></aside>}

function Inicio({go,name,lastContent}){return <><Title sub="Seu próximo conteúdo começa aqui.">Oi, {name||'Imparável'} ✦</Title><Card className="hero"><small>CONTEÚDO DO DIA</small><h2>Uma geração.<br/>Seu dia completo.</h2><p>Stories, Reels e Carrossel conectados, prontos para você usar.</p><Button onClick={()=>go('criar')}>CRIAR MEU CONTEÚDO　›</Button></Card><Card className="impulso"><b>✦</b><div><small>IMPULSO DO DIA</small><h3>{dailyImpulse()}</h3></div></Card><h2>Último conteúdo</h2><Card className="dark row"><div><small>CONTEÚDO COMPLETO</small><h3>{lastContent?.title||'Seu primeiro conteúdo está esperando por você.'}</h3><p>{iconLibrary.stories} Stories • {iconLibrary.reels} Reels • {iconLibrary.carrossel} Carrossel</p></div><Button onClick={()=>go(lastContent?'resultado':'criar')}>{lastContent?'ABRIR':'CRIAR'} ›</Button></Card><Card className="alpha-mini"><div><small>EM BREVE</small><h3>Comunidade Alpha</h3><p>A comunidade dos Imparáveis.</p></div><b>✦</b></Card></>}

function Criar({go,generate,loading}){const [goal,setGoal]=useState('Atrair pessoas');const [subject,setSubject]=useState('');const [way,setWay]=useState('Tenho pouco tempo');return <><button className="back" onClick={()=>go('inicio')}>‹ Voltar</button><Title sub="Você responde o mínimo. Eu preparo tudo.">Faça por mim</Title><Card className="dark"><small>SEU DIA DE CONTEÚDO</small><h2>Seu conteúdo do dia, todo pronto.</h2><p>Stories + Reels + Carrossel, pensados para funcionar juntos.</p></Card><Card className="form"><Question n="1" text="O que você quer conseguir hoje?"/><div className="choices">{['Vender','Atrair pessoas','Passar confiança','Ensinar','Criar conexão','Escolha por mim'].map(x=><button className={goal===x?'selected':''} onClick={()=>setGoal(x)}>{x}</button>)}</div><Question n="2" text="O que você quer divulgar hoje?"/><textarea value={subject} onChange={e=>setSubject(e.target.value)} placeholder="Serviço, produto, música, evento ou mensagem..."/><button className="link" onClick={()=>setSubject('Escolha por mim')}>✦ Não sei. Escolha por mim.</button><Question n="3" text="Como você quer fazer hoje?"/><div className="choices">{['Aparecer e falar','Mostrar sem falar','Não quero aparecer','Tenho pouco tempo'].map(x=><button className={way===x?'selected':''} onClick={()=>setWay(x)}>{x}</button>)}</div><Button onClick={()=>generate({goal,subject,way})}>{loading?'CRIANDO...':'CRIAR MEU CONTEÚDO DO DIA　›'}</Button></Card></>}
...
function Login(){
  ...
  return <div className="login-page"><div className="login-brand"><div className="login-logo">D<sup>✦</sup></div><h1>DESTRAVE</h1><p>by Angladi</p><strong>Seu conteúdo pronto. Seu negócio em movimento.</strong></div><form className="login-box" onSubmit={enter}><small>ÁREA DE MEMBROS</small><h2>Bem-vindo de volta.</h2><p>Entre para criar seu conteúdo do dia.</p><label>E-mail<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="seuemail@exemplo.com" required/></label><label>Senha<input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Sua senha" required/></label>{message&&<div className="login-message">{message}</div>}<Button>{busy?'ENTRANDO...':'ENTRAR ›'}</Button><button type="button" className="forgot" onClick={reset}>Esqueci minha senha</button><a className="whatsapp" href="https://wa.me/" target="_blank" rel="noreferrer">Ainda não tenho acesso • Falar no WhatsApp</a></form></div>
}
